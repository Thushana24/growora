...
const NoiseBackground = () => (
  <div className="group absolute inset-0 z-10 overflow-hidden rounded-(--rounded) mask-b-from-0 opacity-30">
    <svg
...
const ButtonBorder = () => (
  <div className="group absolute inset-0 p-(--padding) [--padding:0.1rem]">
    <div
      className={cn(
        "relative z-20 size-full rounded-[calc(var(--rounded)-var(--padding))] border-[0.08rem] border-white/15 bg-linear-to-b from-white/10 to-white/5 shadow-xs",
    ...
    return (
      ...
      >
        ...
        {!isLoading ? (
          ...
          <Spinner className="size-4 text-(--color-btn-text-disabled) dark:text-(--color-btn-text-disabled)" />
        )}
      </button>
    );
  },
